...
> *Note on Scope:* This article addresses throughput and topology, not global transactional correctness across shards. Once a workflow spans multiple shards, new coordination laws (e.g., 2PC, Sagas) apply.
...
* Prepared statements, temporary tables, and session state (`SET session_replication_role...`) can heavily constrain proxy modes and require complex application-side workarounds.

---

## 📡 7. Observability Signals
To safely operate a sharded multiplexed topology, monitor these critical metrics:
* Active vs. Multiplexed backend connections.
* Shard occupancy and depletion skew (Hot Shards).
* P99 allocation latency *by shard*.
* Proxy queue depth (PgBouncer wait times).
* Failed routing attempts/retries per shard.

---

## 🏛 8. The Global Invariant

**At scale, shard fan-out must be terminated before it reaches the storage layer; otherwise, connection growth outpaces useful throughput.** Application threads should not fan out directly across a heavily sharded topology; a multiplexing or data-access control layer becomes mandatory.

---

## ✨ 9. The "Brick" Summary (Mental Model)
