...
> **Note:** In ultra-hot paths, advanced teams often collapse the selection and claim phases into a tighter single-statement transactional pattern (e.g., using CTEs with `UPDATE ... RETURNING`) to halve network round trips and shrink the lock duration even further.
