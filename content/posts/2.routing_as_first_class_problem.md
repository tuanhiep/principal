...
public class DynamicRoutingEngine {
    ...
    // 💠 THE PIVOT INSIGHT: The framework automatically injects all active plugins.
...
The architecture shifts from a static, tangled maze to a dynamic, declarative pipeline.

```mermaid
graph TD
    A[Incoming Context] --> B(DynamicRoutingEngine)
    B --> C{Injected Plugin List}
    C -->|evaluate supports| D[Plugin A: false]
    C -->|evaluate supports| E[Plugin B: false]
    C -->|evaluate supports| F[Plugin C: true]
    F --> G[execute()]
    G --> H((Result))
...
* **The Mitigation**: You must enforce strict mutually exclusive conditions in the `supports()` method. For added safety, utilize `@Order` to explicitly rank the plugins, removing non-deterministic framework loading behaviors.
