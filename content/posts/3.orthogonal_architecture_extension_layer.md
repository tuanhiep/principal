...
In [Part 1](principal/posts/2.routing_as_first_class_problem), we successfully decoupled our payment routing logic into isolated `ExecutionPlugins`. The Core is now clean. However, in an Enterprise environment, executing the core domain is only 20% of the workload.
